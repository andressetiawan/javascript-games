document.addEventListener("DOMContentLoaded", () => {
  ...
  for (let i = 0; i < squares.length; i++) {
    squares[i].addEventListener("click", () => {
      const isTaken = squares[i].classList.contains("taken");

      if (squares[i + 7].classList.contains("taken") && !isTaken) {
  ...
  function checkBoard() {
    ...
    for (let i = 0; i < winningArray.length; i++) {
      ...
      if (
        ...
        result.textContent = "Gameover! Blue win!";
        ...
        result.textContent = "Gameover! Red win!";
